...
bool I2c::probe(uint8_t addr)
{
        ...
        for (int attempt = 0; attempt < 3; ++attempt) {
            ...
            if (write(addr, &dummy, 1u) == I2C0_OK) {
                return true;
            }
        }
        return false;
...
bool I2c::wait_idle(uint32_t timeout_ms)
    {
        ...
        while (1) {
            ...
            uint32_t fifo_stat = read(I2C0_REG_FIFO);
        ...
        while (1) {
            uint32_t status = read(I2C0_REG_STATUS);
...
int I2c::write(uint8_t addr, const uint8_t *buf, size_t len)
{
    ...
    set_address(addr);
    if (len == 1u) {
        if (!push_data(buf[0])) return I2C0_FIFO_ERROR;    
        asm volatile("" ::: "memory");
        if (!push_cmd(I2C0_CMD_START | I2C0_CMD_WRITE)) return I2C0_FIFO_ERROR;

        // Attendre que la commande soit consommée avant de toucher à l'adresse pour le STOP
        const uint32_t wait_start = millis();
        while ((read(I2C0_REG_FIFO) & I2C0_FIFO_CMD_EMPTY) == 0u) {
            if ((millis() - wait_start) >= I2C0_TIMEOUT_LOOP) {
                return I2C0_TIMEOUT;
            }
        }
        asm volatile("" ::: "memory");
        if (!push_cmd(I2C0_CMD_STOP)) return I2C0_FIFO_ERROR;
        ...
        for (size_t i = 1; i + 1u < len; ++i) {
            ...
            if (!push_data(buf[i])) return I2C0_FIFO_ERROR;
            asm volatile("nop" ::: "memory");
            if (!push_cmd(I2C0_CMD_WRITE)) return I2C0_FIFO_ERROR;
        }
        ...
        if (!push_data(buf[len - 1u])) return I2C0_FIFO_ERROR;
    ...
    if (!wait_idle(I2C0_TIMEOUT_LOOP)) {
        return I2C0_TIMEOUT;
    }
...
int I2c::read(uint8_t addr, uint8_t *buf, size_t len)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        while ((read(I2C0_REG_FIFO) & I2C0_FIFO_RD_EMPTY) != 0u) {
            if ((millis() - start) >= I2C0_TIMEOUT_LOOP) {
                return I2C0_TIMEOUT;
            }
        }
        buf[i] = (uint8_t)(read(I2C0_REG_DATA) & 0xFFu);
    }
